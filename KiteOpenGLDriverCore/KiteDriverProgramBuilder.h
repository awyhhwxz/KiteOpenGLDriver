...
namespace kite_driver
{
	...
	public:
		...
		void SetUniformTexture(const tchar* uniformName, const std::shared_ptr<KiteDriverTexture>& texture);
	private:
		void UniformValueSet(const tchar* uniformName, KiteDriverParaValue * para_value);
		void UniformTextureSet(const tchar* uniformName, KiteDriverTexture* texture, int texture_index);
	protected:
		GLuint _program;
		std::map<tstring, std::shared_ptr<KiteDriverParaValue>> _uniform_value_map;
		std::map<tstring, std::shared_ptr<KiteDriverTexture>> _uniform_texture_map;
	};
